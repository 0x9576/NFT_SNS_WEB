...
function MainPage() {
  ...
  const renderFeeds = feed.map((feed, index) => {
    const location_arr = feed.locationInfo.split(" ");
    return (
      <div id='wrapper' key={feed._id}>
        <div id='header_feed'>
          <a href={'/profile/' + feed.writer}>{feed.writer.substring(0, 6)}...{feed.writer.slice(-4)} </a>
        ...
        <h4>{location_arr[2] + " " + location_arr[3]}</h4>
